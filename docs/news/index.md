...
- [`shoot()`](https://gillescolling.com/texanshootR/reference/shoot.md):
  budgeted exploratory linear-model search across predictor subsets,
  transformations, interactions, sample restrictions, outcome
...
- `restriction` axis: complete-cases pruning, Tukey IQR fence on the
  ...
  restriction.
- `outcome_construction` axis: composite indices, residualisation, ratio
  outcomes, within-group z-scoring.
...
- [`run_log()`](https://gillescolling.com/texanshootR/reference/run_log.md):
  persisted history of recorded runs.
...
- `+iid_rescue` (alias `+iid`): reassign an inconvenient fixed effect to
  an iid random effect when the result is otherwise unwelcome.
...
- `+mechanism_rebuttal` (alias `+intuition`, `+but_biologically`):
  ...
  than engaging with the statistical content. The null simulation is set
  aside because the mechanism is “obvious”.
...
- `+narrow_answer` (alias `+strawman`): answer a weaker version of the
  concern at the wrong level of analysis.

- `+complexity_shield` (alias `+its_complex`): invoke unspecified
  complexity to dismiss a specific narrow concern.

- `+frame_shift` (alias `+oscillate`): oscillate between descriptive and
  causal framing depending on which is being challenged.

- `+matter_of_taste` (alias `+just_one_way`): soften a falsifiable
  concern into a matter of perspective.

- `vocab_tags` includes `aggregation_artifact` and `deflection` fallacy
  tags.

### Output generators

Career-gated output set: locked functions return a status block and
become usable once the tier is reached. Unlocks persist across sessions
via the save state.
